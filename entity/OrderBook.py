...
class OrderBook:
    ...
    def get_orders_for_user(self, symbol, user):
        with self.lock:
            ...
            matched_orders = [
                ...
                if trade['buy_order_id'] == user or trade['sell_order_id'] == user
            ]
    ...
    def add_order(self, symbol, order_type: StockAction, order_id, price, quantity, user=None):
        timestamp = datetime.now()  # Use current timestamp for ordering
        order = Order(order_id, price, quantity, timestamp, user)
        order_book = self._get_order_book(symbol)
    ...
    def modify_order(self, symbol, order_id, new_price, new_quantity):
        with self.lock:
            ...
            for heap_type in [OrderType.BUY_ORDERS, OrderType.SELL_ORDERS]:
                heap = order_book[heap_type]
                for i, (price, timestamp, order) in enumerate(heap):
                    if order.order_id == order_id:
                        order_to_modify = order
                        order_type = heap_type
                        heap[i] = heap[-1]  # Replace with the last element
    ...
    def match_order(self, symbol):
        with self.lock:
            if self.order_books[symbol][OrderType.BUY_ORDERS] and self.order_books[symbol][OrderType.SELL_ORDERS]:
                order_book = self._get_order_book(symbol)
                buy_order = order_book[OrderType.BUY_ORDERS][0][2]  # Peek at the highest buy order
                sell_order = order_book[OrderType.SELL_ORDERS][0][2]  # Peek at the lowest sell order
                ...
                if buy_order.price >= sell_order.price:
                    ...
                    self.append_matched_orders(symbol=symbol, matched_orders=[matched_order])

                    # Update quantities in OrderBook=
                    # Atomic Transactions (Both to be executed together in a pair)
                    self.updateOrder(symbol, OrderType.BUY_ORDERS, trade_quantity)
                    self.updateOrder(symbol, OrderType.SELL_ORDERS, trade_quantity)

    def append_matched_orders(self, symbol, matched_orders):
        for matched_order in matched_orders:
            self.order_books[symbol][OrderType.MATCHED_ORDERS].append(matched_order)

    def updateOrder(self, symbol: str, orderType: OrderType, quantity: int):
